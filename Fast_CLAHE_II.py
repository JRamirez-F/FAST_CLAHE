...
# Java classes via scyjava.jimport
...
# Duplicate input and convert to numpy
...
if is_rgb_single or is_rgb_stack:
    ...
    choice_rgb = JOptionPane.showOptionDialog(
        None,
        "Choose how to apply CLAHE to the RGB image:",
        "RGB CLAHE Mode",
...
out_path=os.path.join(orig_dir,f"{orig_name}_CLAHE.tif")
imwrite(out_path, processed_stack)
print(f"Processed image saved as {out_path}")

try:
    data = ij.io().open(out_path)
    ij.ui().show(data)
    print("Displayed in Fiji.")
except Exception as e:
    print(f"Error showing image in Fiji: {e}")
